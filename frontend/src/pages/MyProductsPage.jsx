...
const MyProductsPage = () => {
  const { fetchUserProducts, userProducts } = useProductStore();
  ...
  const currentProducts = userProducts.slice(startIndex, startIndex + itemsPerPage);
  const startNumber = userProducts.length === 0 ? 0 : startIndex + 1;
  const endNumber = Math.min(startIndex + itemsPerPage, userProducts.length);
  ...
  return (
    <Container maxW={Container.xl} py={0}>
      <Flex align="center" justify="space-between" px="15px">
        ...
        {userProducts.length > 0 && (
          <Text fontSize="14px" fontWeight="bold" color="gray.600" mt="10px" px="15px">
            Showing {startNumber}–{endNumber} of {userProducts.length}
          </Text>
        )}
      </Flex>
      ...
      {userProducts.length > itemsPerPage && (
